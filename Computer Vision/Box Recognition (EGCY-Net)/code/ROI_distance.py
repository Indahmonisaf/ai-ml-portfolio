...
# Open video capture
video_capture = cv2.VideoCapture('A3.mp4')
...
# Define minimum and maximum object sizes (adjust as needed)
min_object_size = 1000  # Adjust as needed
max_object_size = 5000  # Adjust as needed
...
while True:
    ...
    for i, (batch_id, x0, y0, x1, y1, cls_id, score) in enumerate(outputs):
        if score >= 0.5:  # Apply threshold
            ...
            if min_object_size <= object_area <= max_object_size and x0 >= 0 and x0 + x1 <= roi_width and y0 >= 0 and y0 + y1 <= roi_height:
                # Draw bounding box and label on the original frame
                #label = f"Class: {class_id}, Confidence: {confidence:.2f}"
                cv2.rectangle(frame, (int(x0) + roi_x, int(y0) + roi_y), (int(x0 + x1) + roi_x, int(y0 + y1) + roi_y), (0, 255, 0), 2)
                cv2.putText(frame, name, (int(x0) + roi_x, int(y0) + roi_y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    ...
    for class_name, count in class_counts.items():
        ...
        cv2.putText(cropped_frame, total_label, (10, 30 + tambah), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        tambah += 30
